// Tags-input Component scripts -->
/// Tags-input jQuery main function -->
!(function (a) {
  var b = new Array(),
    c = new Array();
          ...
          if ("" != d && 1 != h) {
            if (
              (a("<span>")
                ...
                .append(
                  a("<span>").text(d),
                  a("<a>", { href: "#", title: "Removing tag"}).click(function () {
                    return a("#" + f).removeTag(escape(d));
                  }).append(
                    a("<svg><use xlink:href='../../assets/svg/base-sprite/base-sprite.svg#Warning'></use></svg>")
                  )
                )
      ...
      return (
        this.each(function () {
          if ("undefined" == typeof a(this).attr("data-tagsinput-init")) {
            ...
            var e = a(this).attr("id");
            var inputClassList = a(this).attr('class').split(/\s+/);
            var fieldClassList = ' ';
            var innerInputClassList = ' ';
            a.each(inputClassList, function(_, item){
              if (['tags-input', 'tags-input_small', 'tags-input_large'].indexOf(item) != -1) {
                fieldClassList = fieldClassList + ' ' + item + '__field';
                innerInputClassList = innerInputClassList + ' ' + item + '__input';
              };
            });
            (!e || b[a(this).attr("id")]) &&
              (e = a(this)
            ...
            var i = '<div id="' + e + '_tagsinput" class="tagsinput ' + fieldClassList + '"><div id="' + e + '_addTag">';
            if (
              (f.interactive && (i = i + '<input id="' + e + '_tag" class="' + innerInputClassList + '" value="" data-default="' + f.defaultText + '" />'),
...
/// Tags-input jQuery main function <--
$(function() {
  var tags_accent_1 = ['php', 'java', 'ruby', 'python', 'js', 'json', 'rest', 'api'];
  var tags_accent_2 = ['style', 'font', 'svg', 'size', 'typography', 'image', 'line'];

  jQuery.fn.addColorToTag = function() {
    if($(this).text().search(new RegExp('\\b(' + tags_accent_1.join('|') + ')\\b')) >= 0) {
      $(this).addClass('tag_accent_1');
    };
    if($(this).text().search(new RegExp('\\b(' + tags_accent_2.join('|') + ')\\b')) >= 0) {
      $(this).addClass('tag_accent_2');
    };
  };
  ...
  for (var ti in tag_inputs) {
    var tag_input = tag_inputs[ti];
    (function(tgi){
      $(`.${tgi}`).tagsInput({
        'height': null,
        'width': null,
        'placeholderColor' : '#b3b3b3',
        onChange: function(elem, elem_tags) {$('.tag', elem_tags).each(function(){$(this).addColorToTag();});},
        onAddTag: function(elem, elem_tags) {$('.tag', elem_tags).each(function(){$(this).addColorToTag();});},
      });
    }(tag_input))
  }
});
